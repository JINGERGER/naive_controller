...
| 决策路径 | 触发条件 | 行为 |
|---------|---------|------|
| `EMERGENCY` | 障碍物边缘距离<0.05m | 停止+旋转脱困，超时后退 |
| `STUCK_RECOVERY` | 3秒内移动<1cm | 纯旋转脱困 |
| `STRAIGHT_ROTATE` | 前方>2m畅通 且 角度偏差>8.6° | 纯旋转对准 |
| `STRAIGHT_FORWARD` | 前方>2m畅通 且 角度对准 | 纯直行 |
| `DWA-前方畅通直行` | DWA输出v,w都很小 且 前方畅通 | 强制直行 |
| `DWA-避障旋转` | DWA输出v,w都很小 且 前方有障碍 | 纯旋转避障 |
| `DWA-纯旋转` | DWA输出w大、v小 | 纯旋转 |
| `DWA-纯直行` | DWA输出v大、w小 | 纯直行 |
| `DWA-组合` | DWA输出v,w都较大 | 组合运动 |
| `SIMPLE_ROTATE` | DWA禁用 且 角度偏差大 | 简单旋转 |
| `SIMPLE_FORWARD` | DWA禁用 且 角度对准 | 简单直行 |
...
```bash
python3 dwa_controller.py --ros-args -p debug_performance:=true
```

输出示例：
```
[PERF] 5s统计: Loop=10.0Hz, Scan=2.5ms(n=50), DWA=15.3ms(n=45), Obstacles=120
[CMD_VEL] state=MOVING, linear.x=0.100, angular.z=0.500, path=DWA-纯旋转
```
